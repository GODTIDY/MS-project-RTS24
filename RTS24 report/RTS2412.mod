...
# STAGE-1 VARIABLES
# =====================
var Capacity {i in StorageBus} >= 0;                        #Installed battery energy at bus i          ##2.1
...
# ----- Power balance -----
subject to PowerBalance {i in I, t in T, s in S}:                                                       ##2.3
...
# ----- DC flow -----
subject to FlowDef {l in L, t in T, s in S}:                                                            ##2.4
    flow[l,t,s] = (1/X[l]) * (theta[FromBus[l],t,s] - theta[ToBus[l],t,s]);

subject to LineLimit_lower {l in L, t in T, s in S}: flow[l,t,s] >= -RateAScale * RateA[l];             ##2.13
subject to LineLimit_upper {l in L, t in T, s in S}: flow[l,t,s] <=  RateAScale * RateA[l];             ##2.13

# ----- Lower limit of the unit (with shutdown switch) -----
subject to GenMin {p in P, t in T, s in S}:                                                             ##2.12
    Pg[p,t,s] >= (1-AllowShutdown) * Pmin[p];              

subject to GenMax {p in P, t in T, s in S}:                                                             ##2.12
    Pg[p,t,s] <= Pmax[p];
...
subject to BatterySOC {i in StorageBus, t in T, s in S: t>1}:                                           ##2.9
    SOC[i,t,s] = SOC[i,t-1,s] + eta_c*Charge[i,t,s] - (1/eta_d)*Discharge[i,t,s];

subject to BatteryInitEnd  {i in StorageBus, s in S}: SOC[i,1,s]       = 0.5*Capacity[i];               ##2.11
subject to BatteryFinalSOC {i in StorageBus, s in S}: SOC[i,card(T),s] = 0.5*Capacity[i];               ##2.11

# ----- limit of charging and discharging power ≤ the Curtpeak & SOC range -----
subject to ChargePeak {i in StorageBus, t in T, s in S}:                                                ##2.8
    Charge[i,t,s]    <= Capacity[i]*0.25;

subject to DischargePeak {i in StorageBus, t in T, s in S}:                                             ##2.8
    Discharge[i,t,s] <= Capacity[i]*0.25;
    
subject to DischargePeak1 {i in StorageBus, t in T, s in S}:                                            ##2.14
    Discharge[i,t,s]+WindPg[i,t,s] <= max {tt in T, ss in S} WindPg[i,tt,ss];# / hFactor;    
        
subject to SOCLower {i in StorageBus, t in T, s in S}:                                                  ##2.10
    SOC[i,t,s] >= SOC_min * Capacity[i];

subject to SOCUpper {i in StorageBus, t in T, s in S}:                                                  ##2.10
    SOC[i,t,s] <= SOC_max * Capacity[i];

# ----- Charging source & Upper limit of power rationing -----
subject to ChargeDecompose {i in StorageBus, t in T, s in S}:                                           ##2.6
    Charge[i,t,s] = ChargeCurtail[i,t,s] + ChargeGrid[i,t,s];           

subject to CurtailmentCap {i in StorageBus, t in T, s in S}:                                            ##2.7
    ChargeCurtail[i,t,s] <= CurtailPg[i,t,s];                           

# ----- Battery discharge injection limit -----
subject to PgBatteryMatch {i in StorageBus, t in T, s in S}:                                            ##2.15
    PgBattery[i,t,s] <= Discharge[i,t,s];

# ---------- TotalEnergy ≤ Σ 4×CurtPeak ----------
subject to TotalEnergyCap:                                                                              ##2.5
